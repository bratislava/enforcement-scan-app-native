...
export const ScansAndOffencesApiAxiosParamCreator = function (configuration?: Configuration) {
  return {
     ...
     * @param {ScanControllerCreateOffenceOffenceTypeEnum} offenceType Stringified JSON of RequestCreateOffenceDataDto.
     * @param {boolean} objectiveResponsibility Set to false if the fine was paid on the spot to a police officer; true otherwise. For PAAS offences (offence types O, N, N_B), this value is always true.
     * @param {Array<File>} files Array of jpeg files in base64 format.
     * @param {number} [_long] Longitude of the offence location. If different from the value in associated scan, it indicates the location was manually edited before creating the offence.
     * @param {number} [lat] Latitude of the offence location. If different from the value in associated scan, it indicates the location was manually edited before creating the offence.
     * @param {string} [streetName] Name of the street where the offence occurred. Typically provided only if the location was manually edited and differs from the associated scan.
     * @param {string | null} [udr] UDR code (4-digit number or \\\&quot;0\\\&quot;) for the offence location. Typically provided only if the UDR was manually edited and differs from the associated scan.
     * @param {number} [vehicleId] Vehicle ID from NEV. Used to identify a specific vehicle when multiple vehicles share the same ECV.
     * @param {ResolutionOffenceTypeEnum} [resolutionType]
     * @param {number} [zoneSignPhotoId] ID of zone sign photo in database.
     * @param {*} [options] Override http request option.
     * @throws {RequiredError}
     */
    scanControllerCreateOffence: async (
      scanUuid: string,
      offenceType: ScanControllerCreateOffenceOffenceTypeEnum,
      objectiveResponsibility: boolean,
      files: Array<File>,
      _long?: number,
      lat?: number,
      ...
      // verify required parameter 'offenceType' is not null or undefined
      assertParamExists('scanControllerCreateOffence', 'offenceType', offenceType)
      // verify required parameter 'objectiveResponsibility' is not null or undefined
      assertParamExists(
      ...
      if (offenceType !== undefined) {
        localVarFormParams.append('offenceType', offenceType as any)
      }

      if (objectiveResponsibility !== undefined) {
        localVarFormParams.append('objectiveResponsibility', objectiveResponsibility as any)
      }
      ...
      if (streetName !== undefined) {
        localVarFormParams.append('streetName', streetName as any)
      }

      if (udr !== undefined) {
        localVarFormParams.append('udr', udr as any)
      }
      ...
      if (resolutionType !== undefined) {
        localVarFormParams.append('resolutionType', resolutionType as any)
      }

      if (zoneSignPhotoId !== undefined) {
        localVarFormParams.append('zoneSignPhotoId', zoneSignPhotoId as any)
      }
      if (files) {
 ...
 * @export
 */
export const ScanControllerCreateOffenceOffenceTypeEnum = {
  A: 'A',
  B: 'B',
  C: 'C',
  D: 'D',
...
export type ScanControllerCreateOffenceOffenceTypeEnum =
  (typeof ScanControllerCreateOffenceOffenceTypeEnum)[keyof typeof ScanControllerCreateOffenceOffenceTypeEnum]
/**
 * @export
 */
